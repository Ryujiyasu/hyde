...
## Section P: BYOD × 退職時セキュリティ

### 従来のBYOD問題

```
従来:
  私用スマホにファイルをコピー → 平文で持ち出される
  退職時に回収しようがない
  MDM（モバイルデバイス管理）でリモートワイプ → 私用端末に入れたくない
  → BYODを禁止するか、セキュリティを諦めるか
...
| 問題 | 従来の対策 | hydeの対策 |
|------|----------|-----------|
| 私用端末へのコピー | コピー禁止（DLP） | コピーしてもいい（暗号文） |
| 退職時の持ち出し | リモートワイプ / MDM | 会社PC回収で自動無力化 |
| 社員の反発 | 「私用端末に会社のソフト入れたくない」 | 何もインストール不要 |
| IT部門の負荷 | MDM運用・監視コスト | 回収するだけ |
...
在職中:
  私用スマホ・タブレットでファイルを閲覧
  → 会社PCのTPMが必要なので、会社PCが手元にある時だけ復号可能
  → 自宅でもカフェでも作業できる

退職時:
  会社PCを回収する
  → 私用デバイス上の暗号文は全て復号不可能になる
  → リモートワイプ不要
  → 退職者のデバイスに何もしなくていい
```
...
**BYODとセキュリティが初めて両立する。「コピーを禁止する」のではなく「会社の鍵がなくなれば読めない」。**

---

## Section Q: PC + スマホ分散認証 — パスワードを超える物理認証
...
スマホ（常に持ち歩く）:
  データはない（軽量）
  TPM-B（またはSecure Enclave）に鍵の残り半分

復号:
  PCの前でスマホをかざす → 両方のTPMが揃う → 復号
  席を離れる → スマホがない → PCだけでは復号不可能
```
...
| 認証方式 | 種類 | 弱点 |
|---------|------|------|
| パスワード | 知識 | 忘れる、盗まれる、フィッシングされる |
| 生体認証 | 身体 | 偽造される（写真・型取り）、強制される |
| スマートカード | 所持 | クローン可能、紛失リスク |
| FIDO2/YubiKey | 所持+暗号 | 1デバイス、紛失で全喪失 |
| **TPM分散** | **物理法則** | **コピー不可能（FixedTPM）、忘れない、遠隔で盗めない** |

### 攻撃コストの比較

```
パスワード窃取:        ネットワーク経由で可能（低コスト）
生体偽装:             写真や型取りで可能（中コスト）
スマートカードクローン:  専門機材で可能（中コスト）
TPM 1台の鍵取り出し:   FixedTPMが拒否（不可能）
TPM 1台ごと物理奪取:   その場に行く必要がある（高コスト）
TPM 2台を同時に奪取:   2箇所で同時に奪う必要がある（超高コスト）
```

**「2つの物理デバイスを同時に奪う」は、パスワードやカードの世界には存在しないレベルの攻撃コスト。**

### 日常の運用

```
出社:
  PCの前に座る + スマホが手元にある → 復号可能 → 仕事できる

離席:
  スマホをポケットに入れて離席 → PCだけでは復号不可能
  → 同僚がPCを覗いても暗号文しか見えない

帰宅:
  スマホだけ持ち帰り → データはPCにある → スマホにはデータがない
  → スマホを落としても情報漏洩ゼロ

紛失:
  PCを盗まれた → スマホのTPMがない → 復号不可能
  スマホを落とした → データがない → 復号するものがない
  両方同時に盗まれる → 同じ場所に両方ある瞬間だけ → 席にいる時だけ
```

### 一言で

**パスワードは「知っていること」、生体認証は「であること」、TPM分散は「物理的に持っていること」。物理法則は遠隔で突破できない。**

---

## Section R: VPN不要論 — ネットワークの安全性に依存しない

### VPNの本質的問題

```
VPNがやっていること:
  「このネットワークは安全」というトンネルを作る
  → トンネルの中は平文
  → トンネルに入られたら全部見える
...
| | VPN | hyde |
|---|---|---|
| 前提 | ネットワークが安全であること | ネットワークは安全でなくていい |
| トンネル内 | 平文 | 暗号文 |
| VPN障害時 | 作業停止 | 影響なし |
| 公衆WiFi | 危険 | 問題なし（暗号文だから） |
| 海外出張 | VPN接続必須 | そのまま作業可能 |
| コスト | ライセンス+運用 | ゼロ（hyde自体に含まれる） |
| IT部門の負荷 | VPN管理・障害対応 | なし |

### DLP・VPN・MDM — hydeが不要にするもの

```
従来の企業セキュリティスタック:
  DLP（コピー防止）     → hydeではコピーしても読めない → 不要
  VPN（安全なトンネル） → hydeではトンネルなしで安全 → 不要
  MDM（デバイス管理）   → hydeでは会社PC回収で自動無効化 → 不要

3つ合わせて年間数千万〜数億円のコスト削減。
```

### 一言で

**VPNは「ネットワークを信じる」技術。hydeは「ネットワークを信じない」技術。信じないほうが強い。**
