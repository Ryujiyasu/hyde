# veil

> Unified abstraction layer for hardware-based Trusted Execution Environments (TEE) in Rust.
>
> ハードウェアTEEの統一抽象化レイヤー（Rust）

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Rust](https://img.shields.io/badge/rust-1.70%2B-orange.svg)](https://www.rust-lang.org)

---

## Why veil? / なぜveilが必要か

Data has three states. Two are solved. One is not.

データには3つの状態がある。2つは解決済み。1つは未解決。

| State / 状態 | Threat / 脅威 | Existing Solution / 既存の解決策 |
|------|------|-------------|
| At rest / 保存時 | Disk theft / ディスク盗難 | BitLocker / FileVault |
| In transit / 通信時 | Interception / 通信傍受 | HTTPS / TLS |
| **In use / 実行時** | **Memory access, cloud admin** | **Unsolved ← veil** |

veil binds secrets to **a specific device + a specific person** using TPM (Trusted Platform Module). Even if stored in the cloud, data cannot be decrypted without that person and that device.

veilはTPMを使い、秘密情報を「特定のデバイス＋特定の人物」に紐付けて保護する。クラウドに保存されても、その人物・そのデバイスなしには復号できない。

---

## Quick Start / クイックスタート

```rust
use veil::{self, FallbackPolicy};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut ctx = veil::auto_detect(FallbackPolicy::Deny)?;

    // Protect data (generates Data Key → wraps with TPM → AES-256-GCM encrypt)
    let secret = b"my document encryption key";
    let protected = ctx.protect(secret)?;

    // Serialize and save anywhere (disk, cloud, etc.)
    let json = serde_json::to_string(&protected)?;

    // Decrypt (requires the same TPM)
    let deserialized: veil::ProtectedData = serde_json::from_str(&json)?;
    let recovered = ctx.unprotect(&deserialized)?;
    assert_eq!(recovered, secret);

    // Passphrase backup (for device migration / TPM failure)
    let backup = ctx.backup(&protected, b"my-recovery-passphrase")?;
    let restored = ctx.restore(&backup, &protected.ciphertext, b"my-recovery-passphrase")?;

    Ok(())
}
```

## Architecture / アーキテクチャ

```
┌─────────────────────────────────┐
│       Application               │
│  veil::auto_detect()            │
│  ctx.protect() / unprotect()    │
└────────────┬────────────────────┘
             │
┌────────────▼────────────────────┐
│  veil (facade crate)            │
│  Auto-detects best backend      │
└────────────┬────────────────────┘
             │
┌────────────▼────────────────────┐
│  veil-core                      │
│  TeeBackend trait               │
│  VeilContext, ProtectedData     │
└────────────┬────────────────────┘
             │
┌────────────▼──────────────────────────┐
│  Backend crates                       │
│  ┌──────────┐  ┌──────────────────┐   │
│  │ veil-tpm │  │ veil-software    │   │
│  │ (TPM 2.0)│  │ (fallback stub)  │   │
│  └──────────┘  └──────────────────┘   │
│  ┌────────────────────────────────┐   │
│  │ Phase 2+: veil-tdx, veil-sev  │   │
│  └────────────────────────────────┘   │
└───────────────────────────────────────┘
```

## Key Design: Primary Key + Data Key / 鍵管理の設計

veil uses the **BitLocker pattern** to avoid TPM NV memory exhaustion:

veilはBitLockerパターンを採用し、TPMのNVメモリ枯渇を防ぐ：

1. **Primary Key** (1 per device) — persisted in TPM NV memory (1 slot)
2. **Data Key** (1 per protect call) — generated by TPM RNG, sealed under Primary Key, stored as blob on disk
3. **Encryption** — Data Key used for AES-256-GCM, then zeroized from memory

Primary Key（デバイスに1つ）はTPMのNVメモリに永続化。Data Key（データごと）はTPM乱数で生成し、Primary Keyでラップしてディスクに保存。暗号化にはAES-256-GCMを使用し、使用後はメモリからゼロクリア。

## Recovery / 回復

Passphrase-based backup uses **Argon2id** key derivation + AES-256-GCM:

パスフレーズベースのバックアップは Argon2id 鍵導出 + AES-256-GCM：

```rust
// Backup (before disaster)
let backup = ctx.backup(&protected, b"strong-passphrase")?;
// → save `backup` bytes somewhere safe

// Restore (on new device)
let restored = ctx.restore(&backup, &ciphertext, b"strong-passphrase")?;
let data = ctx.unprotect(&restored)?;
```

## Workspace Structure / ワークスペース構成

```
veil/
├── Cargo.toml              # Workspace root
├── crates/
│   ├── veil/               # Facade: auto_detect() + re-exports
│   ├── veil-core/          # TeeBackend trait, VeilContext, ProtectedData
│   ├── veil-tpm/           # TPM 2.0 backend (tss-esapi)
│   ├── veil-software/      # Software fallback (stub)
│   └── veil-macros/        # #[veil::protect] proc macro (stub)
├── docs/
│   ├── veil-implementation-guide.md
│   └── veil-roadmap.md
└── examples/
```

## Prerequisites / 前提条件

```bash
# Linux
sudo apt install libtss2-dev swtpm swtpm-tools

# Start software TPM for development
mkdir /tmp/swtpm && swtpm socket \
  --tpmstate dir=/tmp/swtpm \
  --ctrl type=tcp,port=2322 \
  --server type=tcp,port=2321 \
  --tpm2 --daemon
swtpm_ioctl --tcp 127.0.0.1:2322 -i

# Windows 11: TPM 2.0 is built-in, no additional install needed
```

## Build & Test / ビルド・テスト

```bash
cargo build --workspace
cargo check --workspace

# Run tests (requires swtpm running)
export TCTI="swtpm:host=127.0.0.1,port=2321"
cargo test --workspace -- --test-threads=1
```

## Roadmap / ロードマップ

| Phase | Target / 対象 | Status / 状態 |
|-------|------|--------|
| **1** | TPM 2.0 (Windows 11 / Linux) | **In progress / 開発中** |
| 2 | Intel TDX, AMD SEV-SNP (Cloud TEE) | Planned / 計画中 |
| 3 | Apple Secure Enclave, ARM TrustZone (Mobile) | Planned / 計画中 |
| 4 | oxi integration, Enterprise SaaS | Planned / 計画中 |

See [docs/veil-roadmap.md](docs/veil-roadmap.md) for details.

## Phase 1 Status / Phase 1 進捗

- [x] TPM connection + session
- [x] Primary Key generation + persistence
- [x] Data Key generation + wrapping
- [x] Seal / Unseal (AES-256-GCM)
- [x] ProtectedData serialization (serde)
- [x] Passphrase recovery (Argon2id)
- [x] VeilContext public API
- [x] auto_detect() facade
- [x] SoftwareBackend stub
- [x] 15 integration tests passing (swtpm)
- [ ] PCR policy binding
- [ ] `#[veil::protect]` macro
- [ ] CI/CD (GitLab CI + swtpm)
- [ ] crates.io publish

## Contributing / コントリビューション

Contributions welcome! / コントリビューション歓迎！

```bash
git clone https://gitlab.com/Ryujiyasu/veil.git
cd veil
cargo build --workspace
cargo test --workspace -- --test-threads=1
```

## License / ライセンス

MIT License

## Author / 著者

Ryuji Yasukochi ([@Ryujiyasu](https://gitlab.com/Ryujiyasu))
