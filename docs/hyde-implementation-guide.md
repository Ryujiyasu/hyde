...
hydeの信頼モデルは「1人が複数のTPMを持ち、鍵を分散する」。データはどこにでもコピーする（暗号文なので問題ない）。守るべきは鍵だけであり、鍵は物理的に離れた場所のTPMに分散され、普段はオフライン。

マルチユーザーOSは「管理者権限」という概念を生み出した。しかしhydeのFixedTPMフラグにより、管理者権限があってもTPMから鍵を取り出すことは不可能。管理者は運用妨害はできるが、情報窃取はできない。

| Phase | アプローチ |
|---|---|
| Phase 1 | TPM分散鍵 + オフラインデバイス + FixedTPM + zeroize + mlock |
| Phase 2 | TDX/SEV-SNPで共有インフラ上でもランタイムメモリ保護を追加 |
...
| モジュール | 限界 | 補完 |
|---|---|---|
| hyde | 管理者は運用妨害（TPM無効化・プロセス停止・ファイル削除）は可能だが、鍵取り出し・データ復号は不可能（FixedTPM）。運用妨害対策は2台目バックアップを推奨 | Phase 2 (TDX/SEV-SNP) でランタイムメモリ保護を追加 |
