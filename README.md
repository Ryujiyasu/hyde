...
|---|---|---|
| **hyde** | Admin can disrupt operations but cannot extract keys or decrypt data (FixedTPM) / 管理者は運用妨害は可能だが、鍵取り出し・データ復号は不可能（FixedTPM） | Phase 2 (TDX/SEV-SNP) for runtime memory protection |
...
| Admin escalation / 管理者権限奪取 | ❌ Out of scope | ⚠ Operational disruption possible, but data theft blocked by FixedTPM / 運用妨害は可能だが情報窃取はFixedTPMが拒否 |
| Physical attacks / 物理攻撃 | ❌ Out of scope | ❌ Out of scope (vendor) |

**Key difference from BitLocker:** hyde's FixedTPM flag means that even with admin privileges, the sealed keys cannot be extracted from the TPM. An admin can disable the TPM, kill the hyde process, or delete encrypted files (operational disruption), but **cannot decrypt existing protected data**. "Can destroy, but cannot steal." To protect against operational disruption (e.g., file deletion), backup to a second device with its own TPM is recommended. Phase 2 (TDX/SEV-SNP) adds runtime memory protection.

**BitLockerとの重要な違い:** hydeのFixedTPMフラグにより、管理者権限があってもTPMから鍵を取り出すことは不可能。管理者はTPMの無効化、hydeプロセスの停止、暗号化ファイルの削除は可能（運用妨害）だが、**既存の暗号化データを復号することはできない**。「壊すことはできるが盗むことはできない」。運用妨害（ファイル削除等）への対策としては、別TPMを持つ2台目のデバイスへのバックアップを推奨する。Phase 2（TDX/SEV-SNP）ではランタイムメモリ保護を追加する。
